/* enrich.server.ts — plain server-only module. All enrichment logic lives
   here so other server functions can call it directly instead of going
   through an RPC stub. */
...
export async function runEnrichCandidate(identityKey: string): Promise<EnrichmentResult> {
    ...
    const mySignals = (rawSignals ?? []).filter(
      (s) => normalize(s.person_or_handle) === identityKey,
    );
...
export async function runEnrichFounder(founderId: string): Promise<EnrichmentResult> {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
